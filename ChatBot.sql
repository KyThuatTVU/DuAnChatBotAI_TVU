...
ADD COLUMN embedding_vector JSON NULL COMMENT 'Vector embedding từ Gemini API (768 dimensions)' AFTER answer_text,
ADD COLUMN embedding_model VARCHAR(100) NULL COMMENT 'Model tạo embedding (vd: text-embedding-004)' AFTER embedding_vector,
ADD COLUMN embedding_updated_at DATETIME NULL COMMENT 'Lần cuối cập nhật embedding' AFTER embedding_model;
...
CREATE TABLE IF NOT EXISTS embedding_cache (
    ...
    query_hash VARCHAR(64) NOT NULL UNIQUE COMMENT 'SHA256 hash của query',
    embedding_vector JSON NOT NULL COMMENT 'Vector embedding',
    model VARCHAR(100) NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_hash (query_hash),
    INDEX idx_created (created_at)
) ENGINE=InnoDB COMMENT='Cache embeddings cho user queries';

-- Bảng log để theo dõi hiệu suất RAG
CREATE TABLE IF NOT EXISTS rag_performance_log (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_query TEXT NOT NULL,
    matched_question_id INT NULL,
    similarity_score FLOAT NULL COMMENT 'Cosine similarity score',
    response_time_ms INT NULL COMMENT 'Thời gian xử lý (ms)',
    method VARCHAR(50) NULL COMMENT 'fulltext, rag, gemini, fallback',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (matched_question_id) REFERENCES questions(id) ON DELETE SET NULL,
    INDEX idx_created (created_at),
    INDEX idx_method (method)
) ENGINE=InnoDB COMMENT='Log hiệu suất của RAG system';


-- 3. Cập nhật bảng rag_performance_log cho hybrid scoring
ALTER TABLE rag_performance_log 
ADD COLUMN hybrid_score FLOAT NULL COMMENT 'Hybrid score tổng hợp' AFTER similarity_score;

ALTER TABLE rag_performance_log 
ADD COLUMN embedding_score FLOAT NULL COMMENT 'Cosine similarity score thuần túy' AFTER hybrid_score;

ALTER TABLE rag_performance_log 
ADD COLUMN keyword_score FLOAT NULL COMMENT 'Keyword overlap score' AFTER embedding_score;

ALTER TABLE rag_performance_log 
ADD COLUMN fulltext_score FLOAT NULL COMMENT 'MySQL FULLTEXT relevance score' AFTER keyword_score;

ALTER TABLE rag_performance_log 
ADD COLUMN confidence_tier VARCHAR(20) NULL COMMENT 'high/medium/low/none' AFTER fulltext_score;

-- 4. Thêm index cho rich_embedding_updated_at
ALTER TABLE questions 
ADD INDEX idx_rich_embedding_updated (rich_embedding_updated_at);

-- 5. Tạo bảng synonym mapping cho query expansion tiếng Việt
CREATE TABLE IF NOT EXISTS query_synonyms (
    id INT AUTO_INCREMENT PRIMARY KEY,
    word VARCHAR(100) NOT NULL COMMENT 'Từ gốc',
    synonym VARCHAR(100) NOT NULL COMMENT 'Từ đồng nghĩa',
    weight FLOAT NOT NULL DEFAULT 1.0 COMMENT 'Trọng số (0-1, 1=đồng nghĩa hoàn toàn)',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE INDEX idx_word_synonym (word, synonym),
    INDEX idx_word (word)
) ENGINE=InnoDB COMMENT='Bảng ánh xạ từ đồng nghĩa cho query expansion';

-- 6. Insert dữ liệu synonym cơ bản cho thư viện
INSERT IGNORE INTO query_synonyms (word, synonym, weight) VALUES
-- Mượn sách
('mượn', 'cho mượn', 1.0),
('mượn', 'thuê', 0.7),
('mượn sách', 'mượn tài liệu', 0.9),
('trả sách', 'trả tài liệu', 0.9),
('trả', 'hoàn trả', 0.9),
-- Thẻ
('thẻ', 'thẻ thư viện', 0.9),
('thẻ', 'card', 0.8),
('làm thẻ', 'đăng ký thẻ', 1.0),
('làm thẻ', 'cấp thẻ', 0.9),
-- Giờ
('giờ mở cửa', 'giờ hoạt động', 1.0),
('giờ mở cửa', 'giờ làm việc', 0.9),
('giờ mở cửa', 'thời gian mở cửa', 1.0),
('mấy giờ', 'giờ mở cửa', 0.8),
('mấy giờ', 'khi nào mở', 0.7),
-- Tra cứu
('tra cứu', 'tìm kiếm', 1.0),
('tra cứu', 'tìm', 0.8),
('tra cứu', 'search', 0.9),
('OPAC', 'tra cứu trực tuyến', 0.9),
-- Phòng/Tầng
('phòng', 'room', 0.8),
('tầng', 'lầu', 1.0),
('tầng', 'floor', 0.8),
...
('sách', 'book', 0.8),
('luận văn', 'đề tài', 0.7),
('luận văn', 'khóa luận', 0.9),
('luận văn', 'thesis', 0.8),
-- Dịch vụ
('photocopy', 'photo', 0.9),
('photocopy', 'sao chụp', 0.8),
('in ấn', 'in', 0.9),
('in ấn', 'print', 0.8),
('wifi', 'internet', 0.7),
('máy tính', 'computer', 0.8),
-- Gia hạn
('gia hạn', 'renew', 0.9),
('gia hạn', 'kéo dài', 0.7),
('gia hạn', 'mượn thêm', 0.8),
-- Phạt
('phạt', 'tiền phạt', 0.9),
('phạt', 'fine', 0.8),
('trễ hạn', 'quá hạn', 1.0),
...
-- Đăng ký
('đăng ký', 'đăng kí', 1.0),
('đăng ký', 'register', 0.8),
('đăng nhập', 'login', 0.9);

SELECT 'Migration RAG v2 completed successfully!' AS status;
...
CREATE TABLE IF NOT EXISTS ai_api_keys (
    ...
    used_tokens_total INT NOT NULL DEFAULT 0,
...
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'ai_enabled');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'ai_system_prompt', 'Bạn là trợ lý thư viện TVU. Trả lời ngắn gọn, đúng trọng tâm. Nếu không chắc, hãy nói bạn chưa có thông tin.', 'text',
       'System prompt cho AI', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'ai_system_prompt');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'ai_max_tokens', '500', 'number', 'Số token tối đa cho mỗi câu trả lời AI', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'ai_max_tokens');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'ai_temperature', '0.2', 'number', 'Độ ngẫu nhiên (temperature) cho AI', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'ai_temperature');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'ai_default_model', '', 'text', 'Model AI mặc định (dùng khi key không có model riêng)', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'ai_default_model');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'match_strict_mode', 'false', 'boolean', 'Ưu tiên độ chính xác khi tìm câu hỏi', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'match_strict_mode');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'match_min_relevance', '0.5', 'number', 'Ngưỡng relevance cho FULLTEXT', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'match_min_relevance');

INSERT INTO chatbot_settings (setting_key, setting_value, setting_type, description, updated_by, created_at, updated_at)
SELECT 'match_min_overlap', '0.3', 'number', 'Tỷ lệ trùng từ khóa tối thiểu', NULL, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM chatbot_settings WHERE setting_key = 'match_min_overlap');

UPDATE ai_api_keys
SET model = 'gemini-2.0-flash', base_url = NULL, last_error = NULL
